...
extern void fbp(TestStruct *t) {
	...
	double ScanR, DecL, startangle, Radius,DeltaY,DeltaL,YCtr,DeltaRX,DeltaRY,RCtrX,RCtrY,RadiusSquare,YL_Offset;
	...
	for (i=0;i<FOILength;i++)
		{
			for (j=0;j<FOIWidth;j++)
			{
				...
				if(tpdata<0)
				{
					...
					t->RecIm[i][j] = -t->RecIm[i][j]/(ProjNum*4); //You can not have the oonstant here
				}//if(tpdata<0)
			}//for (y=0;y<RecSize;y++)
		}//for (x=0;x<RecSize;x++)
	free(VectorS);
